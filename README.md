...
## Platform Used for Project Development

**Front-End**
- HTML5 — page structure and semantic markup
- CSS3 — custom properties (variables) for theming, flexbox/grid layout, responsive design, automatic light/dark mode
- Vanilla JavaScript (ES6) — all interactivity, no framework
- Google Fonts (CDN) — Big Shoulders Display, IBM Plex Sans, IBM Plex Mono

**Back-End**
- None — fully static site, no server or database
- WhatsApp Click-to-Chat (`wa.me` links) used for real message delivery
- Booking form is client-side only (shows a confirmation, doesn't store or send data anywhere)

**Development tools**
- No build step, no package manager, no framework — plain files only
- Can be served with any static file server (e.g. `python3 -m http.server`)
...
## Adding a Real Back-End (optional next step)

If you want the booking form to actually receive and store submissions, common options are:
- **Server**: Node.js + Express, PHP, or a serverless function (Vercel/Netlify Functions, AWS Lambda)
- **Database**: MySQL/PostgreSQL, or a simpler option like Firebase/Supabase
- **Quick form handling**: Formspree, a Google Sheets script, or the WhatsApp Business API for real (not just link-based) messaging
